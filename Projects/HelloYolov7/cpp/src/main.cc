...
int main(int argc, char **argv)
{
    ...
    ret = init_yolov7_model(modelPath, &rknn_app_ctx);
    if (ret != 0)
    {
    ...
    rknn_app_ctx.img_dma_buf.size = rknn_app_ctx.input_mems[0].size;

    //std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
    //inference_yolov7_model(&rknn_app_ctx, &od_results);
